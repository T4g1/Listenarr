...
namespace Listenarr.Application.Services
{
    ...
    {
        public async Task<ImportResult> ImportSingleFileAsync(Download download, Audiobook audiobook, string sourcePath, ApplicationSettings settings, CancellationToken ct = default)
        {
            if (!File.Exists(sourcePath))
            {
                return ImportResult.Error($"Cannot access the given source path: {sourcePath}", sourcePath);
            }
            ...
            try
            {
                if (!FileUtils.IsAudioFile(sourcePath))
                {
                    ...
                    logger.LogWarning("ImportSingleFile: refusing non-audio file {File}", sourcePath);
                    return result;
                }
                ...
                if (File.Exists(sourcePath) && settings.EnableMetadataProcessing)
                {
                    try
                    {
                        extractedMetadata = await metadataService.ExtractFileMetadataAsync(sourcePath);
                        if (extractedMetadata != null)
                        {
                            ...
                            logger.LogDebug("ImportSingleFile: merged extracted metadata for {File}", sourcePath);
                        }
                    }
                    catch (Exception ex) when (ex is not OperationCanceledException && ex is not OutOfMemoryException && ex is not StackOverflowException)
                    {
                        logger.LogWarning(ex, "ImportSingleFile: failed to extract metadata from {File}, using defaults", sourcePath);
                    }
                }
                ...
                var metadataForNaming = BuildNamingMetadata(audiobook, extractedMetadata, metadata.Title);
                logger.LogDebug("ImportSingleFile: Using audiobook metadata for naming (Download {DownloadId}): {Title} by {Artist}",
                    LogRedaction.SanitizeText(download.Id),
                    LogRedaction.SanitizeText(metadataForNaming.Title),
                    LogRedaction.SanitizeText(metadataForNaming.Artist));

                // Prevent importing worse quality than existing files
                try
                {
                    if (audiobook.Files != null && audiobook.Files.Count != 0)
                    {
                        ...
                        if (!IsQualityBetter(candidateQuality, bestExisting, abProfile))
                        {
                            ...
                            result.Message = result.SkippedReason;
                            logger.LogInformation("ImportSingleFile: Skipping import of file {File} for audiobook {AudiobookId} because candidate quality '{Candidate}' is not better than existing '{Existing}'", LogRedaction.SanitizeText(sourcePath), audiobook.Id, candidateQuality, bestExisting);
                            return result;
                        }
                    }
                }
                ...
                {
                    logger.LogDebug(ex, "ImportSingleFile: Failed to evaluate quality for {File}", sourcePath);
                }
                ...
                {
                    throw new InvalidOperationException($"Audiobook {audiobook.Id} basepath should not be empty");
                }

                string basePathForFile = audiobook.BasePath;
                string filenamePattern = filePattern;

                var folderRelative = fileNamingService.ApplyNamingPattern(folderPattern, variables, treatAsFilename: false);
                ...
                var patternAllowsSubfolders = filenamePattern.IndexOf("DiskNumber", StringComparison.OrdinalIgnoreCase) >= 0
                    || filenamePattern.Contains("ChapterNumber", StringComparison.OrdinalIgnoreCase)
                ...
                var filename = fileNamingService.ApplyNamingPattern(filenamePattern, variables, true);
                ...
                result.Success = await fileMover.PerformActionOn(action, sourcePath, destinationPath);
            ...
            catch (Exception exception) when (exception is not (OperationCanceledException or OutOfMemoryException or StackOverflowException))
            {
                ...
                logger.LogWarning(exception, "ImportSingleFile: unexpected failure for {File}", sourcePath);
            }
        ...
        public async Task<List<ImportResult>> ImportFilesFromDirectoryAsync(Download download, Audiobook audiobook, IEnumerable<string> files, ApplicationSettings settings, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                foreach (var file in orderedFiles)
                {
                    ...
                    try
                    {
                        ...
                        {
                            logger.LogWarning(ex, "ImportFilesFromDirectory: Failed to create AudiobookFile for imported file {File}", file);
                        }
                    }
                    catch (Exception ex) when (ex is not OperationCanceledException && ex is not OutOfMemoryException && ex is not StackOverflowException)
                    {
                        res.Success = false;
                        res.Message = ex.Message;
                        logger.LogWarning(ex, "ImportFilesFromDirectory: Failed processing file in directory import: {File}", file);
                    }
            ...
            {
                logger.LogWarning(ex, $"ImportFilesFromDirectory: Failed to import files from directory for download {download.Id}");
            }

            return results;
        }

        public Task<ImportResult> ReprocessExistingFileAsync(Download download, Audiobook audiobook, string sourcePath, ApplicationSettings settings, CancellationToken ct = default)
        {
            // For reprocessing we can reuse ImportSingleFileAsync semantics
            return ImportSingleFileAsync(download, audiobook, sourcePath, settings, ct);
        }
